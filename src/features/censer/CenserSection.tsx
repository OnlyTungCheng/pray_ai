...
import { getIncenseBurnState, type IncensePhase } from "./incense-state";
...
const BASIC_ALTAR_RECEPTACLE = {
  ...
  top: 0.15,
  height: 0.16,
  // The positioned stick root has no height: this anchor is the centre of the
  // bowl opening, not the top edge of the artwork's drop target.
  stickBaseY: 0.8,
};
    ...
    >
      ...
      <g transform="translate(-3.2424745,148.9786785)">
        ...
        <path
          ...
          style={{
            pointerEvents: "all",
            cursor: "url(/emoji/emoji_u1f3b6.svg), crosshair",
          }}
        />
        <ellipse
          stroke="black"
  ...
  return (
    <button
      ...
      className={`-z-10 flex flex-col items-center justify-end gap-2 p-1 text-[2.5vh] font-serif h-1/3 self-end origin-bottom drop-shadow-2xl border-b-4 hover:[--my-rotate:84deg] active:[--my-rotate:50deg] rounded-sm transition-all JossBox_anim__q9MLn ${
        isRemix
          ? "border-pink-900 text-pink-300"
          : "border-[#300] text-amber-500"
      }`}
      style={
        {
          background: isRemix
    ...
    >
      ...
      />
      <div className="px-1 font-bold text-amber-300">
        {isRemix ? "🎧" : "福"}
      </div>
    </button>
  );
...
interface JossStickProps {
  ...
  onDragTargetChange?: (el: HTMLDivElement | null) => void;
  isRemix?: boolean;
}
...
function TraditionalIncenseSprite({ phase }: { phase: IncensePhase }) {
  return (
    <div className={`joss-sprite is-${phase}`} aria-hidden="true">
      {phase !== "unlit" && phase !== "expired" && (
        <div className="joss-ember-tip" data-incense-tip={phase} />
      )}
    </div>
  );
}
...
function JossStick({
  ...
  useEffect(() => {
    const state = getIncenseBurnState(pos, Date.now());
    if (state.phase === "unlit" || state.phase === "expired") return;
    const interval = window.setInterval(() => setNow(Date.now()), 1000);
    return () => window.clearInterval(interval);
  }, [pos]);

  const burnState = getIncenseBurnState(pos, now);
  const isBurning =
    burnState.phase !== "unlit" && burnState.phase !== "expired";
  ...
  const style: React.CSSProperties & Record<string, string | number> = {
    ...
    "--burn-progress": burnState.progress,
    "--stick-scale": Math.max(0.36, 0.36 + burnState.progress * 0.64),
  };
  ...
  const handleMove = useCallback(
    (e: MouseEvent | TouchEvent) => {
      ...
      onDragTargetChange?.(stickRefs.current[0]);
    },
    [onDragTargetChange],
  );
  ...
  }, [handleMove, onDrop, onDragTargetChange, pos]);
  ...
  const handleStart = useCallback(
    (e: React.MouseEvent | React.TouchEvent) => {
      ...
      onDragTargetChange?.(primaryEl);

      window.addEventListener(isTouch ? "touchmove" : "mousemove", handleMove, {
        passive: false,
      });
      window.addEventListener(isTouch ? "touchend" : "mouseup", handleEnd);
    },
    [handleMove, handleEnd, onDragTargetChange],
  );
    ...
    <>
      {pos.num > 1 && (
        <div
          ...
          ref={(el) => {
            stickRefs.current[1] = el;
          }}
        >
          <TraditionalIncenseSprite phase={burnState.phase} />
        </div>
      )}
      {pos.num > 2 && (
        <div
...
export default function CenserSection({
  sticks,
  onAddStick,
  themeMode,
}: CenserSectionProps) {
  const [leftHand, setLeftHand] = useState<IncenseStick>({
    x: 0,
    y: 0,
    z: 0,
    num: 0,
  });
  const [rightHand, setRightHand] = useState<IncenseStick>({
    x: 0,
    y: 0,
    z: 0,
    num: 0,
  ...
  const updateHandStick = (
    ...
    const xPos =
      (buttonEl.offsetLeft + Math.ceil(buttonEl.offsetWidth / 2)) / parentWidth;
    const yPos =
      (buttonEl.offsetTop + Math.ceil(buttonEl.offsetHeight / 2)) /
      parentHeight;
  ...
  const getDropPosition = useCallback(
    (draggedEl: HTMLDivElement | null) => {
      ...
      let relativeX =
        draggedEl.offsetLeft - offsetLeft - receptacle.left * offsetWidth;
      ...
      const isInsideY =
        ...
        draggedEl.offsetTop - draggedEl.offsetHeight <
          offsetTop + (receptacle.top + receptacle.height) * offsetHeight;

      return isInsideX && isInsideY ? { relativeX, receptacle } : false;
    },
    [isRemix],
  );
  ...
  const handleDropTargetChange = useCallback(
    (draggedEl: HTMLDivElement | null) => {
      const isInside = Boolean(getDropPosition(draggedEl));
      setIsDropTargetActive((previous) =>
        previous === isInside ? previous : isInside,
      );
    },
    [getDropPosition],
  );
  ...
  const handleDrop = useCallback(
    (pos: IncenseStick, draggedEl: HTMLDivElement | null) => {
      const dropPosition = getDropPosition(draggedEl);
      if (dropPosition) {
        const newStick: IncenseStick = {
          ...pos,
          x: dropPosition.relativeX,
          y: dropPosition.receptacle.stickBaseY + 0.03 * Math.random(),
  ...
  return (
    <div className="relative pt-2">
      ...
      <div
        className="flex justify-center shrink-0 h-[28vh] md:h-[30vh] perspective select-none pointer-events-none [&>*]:pointer-events-auto"
        ref={containerRef}
      >
        ...
        <JossBox
          ...
          onClick={(e) =>
            setLeftHand(
              updateHandStick(leftHand, e.currentTarget, containerRef.current),
            )
          }
        />
        ...
        >
          ...
          <div
            className={`absolute bottom-0 left-1/2 -translate-x-1/2 w-48 h-20 rounded-full blur-[40px] opacity-60 mix-blend-screen pointer-events-none -z-10 transition-colors duration-700 ${
              isRemix
                ? "bg-fuchsia-500 shadow-[0_0_60px_#ec4899]"
                : "bg-amber-600 shadow-[0_0_50px_#d97706]"
            }`}
          />

          {isRemix ? (
            <img
              id="censer-altar-image"
          ...
          <div className="absolute -top-[50vh] left-0 w-full h-[calc(100%+50vh)] pointer-events-none z-25">
            <RealSmokeEngine isRemix={isRemix} />
          </div>

          <div
            aria-label="Incense / Sparkler receptacle"
            className={`absolute top-0 incense-drop-zone ${isDropTargetActive ? "is-active" : ""}`}
            style={{
          ...
          {!isRemix && (
            <div className="censer-mouth-occluder" aria-hidden="true" />
          )}
        </div>
        ...
        <JossBox
          ...
          onClick={(e) =>
            setRightHand(
              updateHandStick(rightHand, e.currentTarget, containerRef.current),
            )
          }
        />
        ...
        {leftHand.num > 0 && (
          <JossStick
            ...
            draggable
            onDrop={handleDrop}
            onDragTargetChange={handleDropTargetChange}
            isRemix={isRemix}
          />
        )}
        {rightHand.num > 0 && (
